...
void LCD_DrawCheckerPiece(uint8_t piece_num, uint8_t col_num, uint8_t row_num, bool isKinged, int color)
{
  ...
  if(isKinged){
    UG_DrawKingEmblem(row_num, col_num, C_WHITE);
    kingCheck = 1;
  }
  char pcnum_buff[4];
  piece_num++;
  ...
  if(piece_num > 9){
    if(kingCheck == 1){
      LCD_PutStr(9 + (row_num * 30), 12 + (col_num * 30), pcnum_buff, DEFAULT_FONT, C_BLACK, C_WHITE);
    } else {
      LCD_PutStr(9 + (row_num * 30), 12 + (col_num * 30), pcnum_buff, DEFAULT_FONT, C_WHITE, color);
    }
  } else {
    if(kingCheck == 1){
      LCD_PutStr(12 + (row_num * 30), 12 + (col_num * 30), pcnum_buff, DEFAULT_FONT, C_BLACK, C_WHITE);
    } else {
